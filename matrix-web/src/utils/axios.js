...
class HttpRequest {
  ...
  request(options) {
    ...
    options.params = _.omitBy(options.params, v => _.isUndefined(v) || _.isNull(v));
    if (!(options.data instanceof Array)) {
      options.data = _.omitBy(options.data, v => _.isUndefined(v));
    }
    ...
    if (globalConfig.SIGN_FLAG) {
      ...
      let signStr = md5(`${token}${params}${data}${timeStamp}`);
      if (options.method === 'get') {
        signStr = md5(`${token}${params}${timeStamp}`);
      }
